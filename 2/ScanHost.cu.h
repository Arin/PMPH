...
void scanExc(    unsigned int  block_size,
    ...
    num_blocks = ( (d_size % block_size) == 0) ?
                    ...
                    d_size / block_size + 1 ;
    T *d_tmp_out;
    cudaMalloc((void**)&d_tmp_out, num_blocks*sizeof(T));

    scanIncKernel<OP,T><<< num_blocks, block_size, sh_mem_size >>>(d_in, d_tmp_out, d_size);
    cudaThreadSynchronize();
    shiftRightByOne<T><<< num_blocks, block_size, sh_mem_size >>>(d_tmp_out, 
                                                                  d_out, 
                                                                  OP::identity(), 
                                                                  d_size);
    cudaThreadSynchronize();
